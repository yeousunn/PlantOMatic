...
<cfcomponent name="DefaultXmlBeanFactory" 
	...
	<cffunction name="loadBeanDefinitions" access="public" returntype="void"
		...
		<cfif isDefined("arguments.XmlBeanDefinitions.beans.bean")>
			...
			<cfloop from="1" to="#ArrayLen(beans)#" index="beanIx">
				...
				<cfif 
					...
					(NOT 
						...
						AND 
							...
							OR
								(StructKeyExists(beanAttributes, 'isAbstract')
								AND
								beanAttributes.isAbstract
								)
							)
						)
					)>
				...
				<!--- look for isAbstract flag, and parent bean def --->			
				<cfif StructKeyExists(beanAttributes,'isAbstract')>
					<cfset isAbstract = beanAttributes.isAbstract />
				<cfelse>
					<cfset isAbstract = false />
				</cfif>
	...
	<cffunction name="createBeanDefinition" access="public" returntype="void" output="false"
		...
		<cfargument name="isAbstract" type="boolean" default="false" required="false" />
		...
		<cfset variables.beanDefs[arguments.beanID].setIsAbstract(arguments.isAbstract) />
