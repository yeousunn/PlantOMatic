...
<cffunction name="createListenerFunctionContent" output="false">
	...
	<cfset var functionName = listenerFunctionNameFor(eventName) />
	<cfset var content = "" />
	
	<cfoutput>
	<cfsavecontent variable="content">
	<cgfunction name="#functionName#" output="false" hint="I am a message listener function generated for the ""#eventName#"" event.">
...
<cffunction name="controllerHasFunction" output="false">
	...
	<cfloop from="1" to="#arrayLen(md.functions)#" index="i">
		<cfif md.functions[i].name eq functionName>
			<cfreturn true />
		</cfif>
	</cfloop>
